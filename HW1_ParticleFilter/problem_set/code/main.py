...
def init_particles_freespace(num_particles, occupancy_map):
    ...
    # Find free space (unoccupied regions, where occupancy_map == 0)
    free_space = np.argwhere(occupancy_map == 0)
    
    if len(free_space) < num_particles:
        raise ValueError("Not enough free space to initialize all particles")
    
    # Randomly sample `num_particles` positions from free space
    selected_indices = np.random.choice(len(free_space), num_particles, replace=False)
    free_positions = free_space[selected_indices]
    
    # Convert map indices to world coordinates (assuming 10 pixels per meter)
    x_vals = free_positions[:, 1] * 10.0  # Convert column index to x world coordinate
    y_vals = free_positions[:, 0] * 10.0  # Convert row index to y world coordinate
    ...
    # Store in the particle matrix
    X_bar_init[:, 0] = x_vals
    X_bar_init[:, 1] = y_vals
    X_bar_init[:, 2] = theta_vals
    X_bar_init[:, 3] = weights
