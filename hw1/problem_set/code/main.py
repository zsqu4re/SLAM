...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='results1')
    ...
    # Load the precalculated ray-casting lookup table
    lookup_table = np.load('data/lookup_table2.npy')
    sensor_model = SensorModel(occupancy_map, lookup_table)
    ...
    for time_idx, line in enumerate(logfile):
        ...
        for m in range(0, num_particles):
            ...
            # If the particle is in obstacle, sensor model is not needed and the particle is skipped
            if occupancy_map[int(x_t1[1]/10.0), int(x_t1[0]/10.0)] == 1.0:
                X_bar_new[m, :] = np.hstack((x_t1, 0)) # The weight set to 0
                continue
        ...
        RESAMPLING
        """
        # O(Odometry) updates only the motion and does not affect the weights, which should not be resampled.
        # L calculated the sensor probability w_t and resampling can be done to remove low weight particles.
        if meas_type == "L": 
            X_bar = resampler.low_variance_sampler(X_bar)

        if time_idx % 2 == 0 or time_idx == 1:
            if args.visualize:
                visualize_timestep(X_bar, time_idx, args.output)

    generate_video.generate_gif("./results1")
