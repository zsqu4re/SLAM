...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
    ...
    for i in range(n_odom):
        ...
        b[2 + 2 * i] = sqrt_inv_odom[0, 0] * odoms[i, 0]
        b[2 + 2 * i + 1] = sqrt_inv_odom[1, 1] * odoms[i, 1]
    ...
    for i in range(n_obs):
        ...
        A[2 + 2 * n_odom + 2 * i, 2 * pose_id] = -1
        A[2 + 2 * n_odom + 2 * i, 2 * n_poses + 2 * landmark_id] = 1
        A[2 + 2 * n_odom + 2 * i + 1, 2 * pose_id + 1] = -1
        A[2 + 2 * n_odom + 2 * i + 1, 2 * n_poses + 2 * landmark_id + 1] = 1

        b[2 + 2 * n_odom + 2 * i] = sqrt_inv_obs[0, 0] * x
        b[2 + 2 * n_odom + 2 * i + 1] = sqrt_inv_obs[1, 1] * y
